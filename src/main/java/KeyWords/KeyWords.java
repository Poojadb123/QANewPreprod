...
public class KeyWords extends BaseClass {
	...
	String sheetName = "keywords";// Read the data from excel sheet1
	Map<String, Map<String, String>> customer = ref.getExcelAsMap(sheetName);
	//Map<String, Object> customer = ref.exceldata(sheetName);
	
	//Map<String, Map<String, String>> customer1 = ref.getExcelAsMap(sheetName);
...
//	String FileName = (String) customer.get("FileName");
	...
	@FindBy(xpath = "//div/div/form/div[1]/div[4]/div/div/div[1]/div/p")
	WebElement KeywordsReqFieldMsg;
	...
	@FindBy(xpath = "//div/form/div[2]/button[2]")
	WebElement AddBtn;
	...
	@FindBy(css = "div.MuiDialog-container.MuiDialog-scrollPaper.css-ekeie0 > div > div > div > form > div.MuiGrid-root.MuiGrid-container.MuiGrid-spacing-xs-3.css-1h77wgb > div:nth-child(4) > div > svg.MuiSvgIcon-root.MuiSvgIcon-fontSizeMedium.css-1ywo9w2")
	WebElement AddBtn1;
	...
	public String AddKeywords() throws InterruptedException, IOException {
		...
		for (int e = 1; e < excelRows + 1; e++) 
		{
			...
			String scate = (String) customer.get(ind).get("SelectCategory");
			String kw = (String) customer.get(ind).get("Keywords");
	...
	//	Thread.sleep(5000);
		if (AddKeywordsBtn.isDisplayed()) {
			...
			List<WebElement> CustomerList = driver
					.findElements(By.xpath("//*[@id='menu-customerOrganisation']/div[3]/ul/li"));
			if (SCustomer.isDisplayed()) {
				// State.click();
				int NoOfOrg = CustomerList.size();
				int i = 0;
